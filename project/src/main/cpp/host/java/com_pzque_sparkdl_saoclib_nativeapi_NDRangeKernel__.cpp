...
static KernelArgSignature makeArgLimit(JNIEnv *env, jobject arg_limit_jobject);
...
JNIEXPORT jlong JNICALL Java_com_pzque_sparkdl_saoclib_nativeapi_NDRangeKernel_00024_newInstance
    ...
    scoped_array<KernelArgSignature> arg_limits(num_args);
    ...
    /* construct object */
    auto image = reinterpret_cast<CLBinary *>(image_handle);
    auto device = reinterpret_cast<cl_device_id>(device_handle);
    auto ret = reinterpret_cast<jlong>(new NDRangeKernel(image,
                                                         device,
                                                         kernel_name,
                                                         arg_limits.get(),
                                                         num_args));
...
JNIEXPORT void JNICALL Java_com_pzque_sparkdl_saoclib_nativeapi_NDRangeKernel_00024_call
    ...
    for (unsigned i = 0; i < num_args; i++) {
        ...
        args[i] = arg;
        auto mode = arg->getSignature().getMode();
        if (isOutput(mode)) {
    ...
    int work_dim = env->GetArrayLength(global_work_size_list_jarray);

    /* fetch global work size and local work size*/
    size_t *global_work_size_list = NULL;
    size_t *local_work_size_list = NULL;
    ...
    global_work_size_list = new size_t[work_dim];
    for (int i = 0; i < work_dim; i++) {
        jlong temp = 0;
        env->GetLongArrayRegion(global_work_size_list_jarray, i,
                                1, &temp);
        global_work_size_list[i] = temp;
    }
    ...
    if (local_work_size_list_jarray != NULL) {
        ...
        local_work_size_list = new size_t[work_dim];
        for (int i = 0; i < work_dim; i++) {
            jlong temp = 0;
            env->GetLongArrayRegion(local_work_size_list_jarray, i,
                                    1, &temp);
            local_work_size_list[i] = temp;
        }
    }
    kernel->call(work_dim, global_work_size_list, local_work_size_list, args, num_args);
    if (global_work_size_list) {
        delete[] global_work_size_list;
    }
    if (local_work_size_list) {
        delete[] local_work_size_list;
    }
    ...
    for (int index:output_index_list) {
        ...
        writeJArray(env, arg_value_jobject, arg, arg->getSignature().getElemTypeID());
    }
...
static KernelArgSignature makeArgLimit(JNIEnv *env, jobject arg_limit_jobject) {
    ...
    switch (arg_type_id) {
        ...
        case NativeTypeID::c_double:
            return KernelArgSignature::primitiveFromTypeID(arg_mode, arg_type_id);
        case NativeTypeID::c_array: {
            ...
            return KernelArgSignature::arrayFromTypeID(arg_mode, NativeTypeID(elem_type_id), array_length);
        }
    }
}
...
static KernelArg *makeArg(JNIEnv *env, jobject arg_jobject) {
    ...
    switch (arg_type_id) {
        case NativeTypeID::c_byte: {
            ...
            return new ByteArg(byte_value, arg_mode);
        ...
        case NativeTypeID::c_short: {
            ...
            return new ShortArg(short_value, arg_mode);
        ...
        case NativeTypeID::c_int: {
            ...
            return new IntArg(int_value, arg_mode);
        }
